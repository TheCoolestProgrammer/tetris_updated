...
class Future_field():
    ...
    def draw(self, coords):
        ...
        pygame.draw.rect(screen, self.line_color_border,
                         (x1, y1, self.cell_size * self.width, self.cell_size * self.height), self.line_size_border)

        for i in coords:
            pygame.draw.rect(screen, (random.randint(50, 255), random.randint(50, 255), random.randint(50, 255)), (
            x1 + self.cell_size * (i[0] - 1), y1 + self.cell_size * i[1], self.cell_size, self.cell_size))
...
class Field():
    ...
    def rotating(self, shape):
        ...
        for i in range(len(shape.coords)):
            ...
            if len(self.field[0]) > new_x >= 0 and new_y < len(self.field) and (
                    self.field[new_y][new_x] == 0 or self.field[new_y][new_x] == 1):
    ...
    def has_obstacle(self, direction, shape):
        if direction == "down":
            ...
            for cords in shape.coords:
                if cords[0] == len(self.field[0]) - 1 or (
                        self.field[cords[1]][cords[0] + 1] != 0 and self.field[cords[1]][cords[0] + 1] != 1):
                    return True
            return False
        elif direction == "left":
            for cords in shape.coords:
